class EventsController < ApplicationController
  ...
  def index
    ...
    @events.each do |event|
      @allEvents << {:id => event.id, :title => event.title, :description => event.description, :start => "#{event.starts_at.iso8601}", :end => "#{event.ends_at.iso8601}", :allDay => event.all_day, :recurring => false}
    end
